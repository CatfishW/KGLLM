"""
Evaluation WITHOUT reranker - use pure retrieval results.
Previous analysis shows retrieval alone gives 90% Hits@10.
"""
import sys
...
def run_evaluation():
    print("=" * 60)
    print("NO RERANKER - Pure Retrieval + Entity Extraction")
    print("=" * 60)
    ...
    print(f'\nProcessing {len(test_data)} samples (no reranker)...')
    
    for sample in tqdm(test_data):
        if not sample.answer_entities:
            continue
            
        # 1. Direct retrieval - top 10
        ...
        # 3. Extract entities from top-5 paths
        all_candidates = []
        context_lines = []
        ...
        # 4. LLM answer selection
        if unique_candidates:
            prompt = f"""Question: {sample.question}

Knowledge Graph paths and entities:
{context}

Candidates: {', '.join(unique_candidates[:12])}

Select ONLY the entity name(s) that answer the question, separated by comma.

Answer:"""
        else:
            prompt = f"""Question: {sample.question}

Paths: {', '.join(retrieved[:3])}
...
Answer:"""
    ...
    print(f'\n{"="*60}')
    print(f'RESULTS ({count} samples) - NO RERANKER')
    print(f'{"="*60}')
