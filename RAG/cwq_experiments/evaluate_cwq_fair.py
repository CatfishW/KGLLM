"""
Fair CWQ Evaluation (No Oracle).
1. Retrieve Top-10 paths (Entity-Enhanced).
2. Extract candidate entities from THESE paths.
3. LLM selects answer from these candidates.
"""
import sys
...
def extract_entities_from_path(path_str):
    """
    Extract entities from an Entity-Enhanced Path string.
    Format: E1 -> R1 -> E2 -> R2 -> E3
    Entities are at even indices: 0, 2, 4...
    """
    if not path_str:
        return []
    parts = path_str.split(' -> ')
    # Take every 2nd element starting from 0
    return parts[::2]

def run_fair_evaluation():
    print("=" * 60)
    print("CWQ FAIR Evaluation (No Oracle Candidates)")
    print("=" * 60)
    
    train = pd.read_parquet('Data/CWQ/shortest_paths/train.parquet')
    test = pd.read_parquet('Data/CWQ/shortest_paths/test.parquet')
    
    # Build corpus with Entity-Enhanced Paths
    all_paths = set()
    ...
    print("Loading retriever from ./RAG/models/finetuned_retriever_cwq_v2_fix ...")
    ...
    recall = {10: 0} # Is answer in top-10 path candidates?
    count = 0
    ...
    for _, row in tqdm(test_data.iterrows(), total=len(test_data)):
        ...
        # 1. Path Hits Calculation
        gt_paths = set(p.lower() for p in parse_cwq_paths(row['shortest_gt_paths']))
        best_match_path = None # Only used for checking match, NOT for prompt
        
        for k in [1, 3, 5, 10]:
            if any(p.lower() in gt_paths for p in retrieved[:k]):
                hits[k] += 1
        
        # 2. Extract Candidates from RETRIEVED paths
        retrieved_entities = []
        for p in retrieved: # Top 10 paths
            ents = extract_entities_from_path(p)
            for e in ents:
                if e not in retrieved_entities: # Preserve order, dedupe
        ...
        # 3. Prompt (Using Top 5 paths for context, Top 20 entities for candidates)
        context_lines = []
...
Instructions:
- Select the ONE entity that best answers the question from the candidates
...
Answer:"""
        ...
        if count % 100 == 0:
            print(f"Step {count}: F1={f1_sum/count:.3f}, Hits@1={hits[1]/count:.3f}, Recall={recall[10]/count:.3f}")

    print(f"\n{'='*60}")
    print(f"CWQ FAIR RESULTS ({count} samples)")
    print(f"{'='*60}")
